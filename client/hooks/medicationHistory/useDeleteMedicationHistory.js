...
async function deleteMedicationData(record_id){
    ...
    try {
        ...
        const response = await fetch(`${ENDPOINT}/api/patient/delete/${id}/medication_history/${record_id}/`, {
            method: 'PUT',
            headers: {
...
document.getElementById('delete-medication-record').addEventListener('click', (event) => {
    // Show the modal
    const modal = document.getElementById('confirm-deletion-modal');
    const medication_form = document.getElementById('add-form')

    modal.classList.remove('hidden');
    medication_form.classList.add('blur-sm', 'pointer-events-none', 'opacity-50')

    const record_id = new URLSearchParams(window.location.search).get('edit_id');

    // Handle confirmation
    document.getElementById('confirm-delete-btn').onclick = async () => {
        await deleteMedicationData(record_id);

        sessionStorage.setItem('toastMessage', 'Record Successfully Deleted');
        sessionStorage.setItem('toastType', 'success');

        medication_form.classList.remove('blur-sm', 'pointer-events-none', 'opacity-50')
