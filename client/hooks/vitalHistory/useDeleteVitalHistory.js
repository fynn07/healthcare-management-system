...
async function deleteVitalData(record_id){
    ...
    try {
        ...
        const response = await fetch(`${ENDPOINT}/api/patient/delete/${id}/vital_history/${record_id}/`, {
            method: 'PUT',
            headers: {
...
document.getElementById('delete-vital-record').addEventListener('click', (event) => {
    // Show the modal
    const modal = document.getElementById('confirm-deletion-modal');
    const vital_form = document.getElementById('add-formvital')

    modal.classList.remove('hidden');
    vital_form.classList.add('blur-sm', 'pointer-events-none', 'opacity-50')

    const record_id = new URLSearchParams(window.location.search).get('edit_id');

    // Handle confirmation
    document.getElementById('confirm-delete-btn').onclick = async () => {
        await deleteVitalData(record_id);

        sessionStorage.setItem('toastMessage', 'Record Successfully Deleted');
        sessionStorage.setItem('toastType', 'success');

        vital_form.classList.remove('blur-sm', 'pointer-events-none', 'opacity-50')
